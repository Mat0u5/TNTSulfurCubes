...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [TNTSulfurCubes version `$version` is out!](https://modrinth.com/mod/tntsulfurcubes/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [TNTSulfurCubes version `$version` is out!](https://modrinth.com/mod/tntsulfurcubes-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
